...
def login(request):
    try:
        ...
        if not iu_id:
            return Response({'status': 'error', 'message': 'IU domain not found.'},
                            status=status.HTTP_404_NOT_FOUND)

        try:
            user = CustomUser.objects.get(mobile_number=mobile_number, is_active=True, iu_id=iu_id)
        except CustomUser.DoesNotExist:
            return Response({'status': 'error', 'message': 'Invalid mobile number or user not found.'},
        ...
        if not check_password(password, user.password):
            return Response({'status': 'error', 'message': 'Incorrect password.'},
                            status=status.HTTP_401_UNAUTHORIZED)
        try:
            user_profile = UserPersonalProfile.objects.get(user=user, is_active=True, iu_id=iu_id)
            role_mappings = RoleMapping.objects.filter(user=user).select_related('role').order_by('role__created_at') 
            roles = list({role_mapping.role.name for role_mapping in role_mappings})

        except Exception as e:
            return Response({'status': 'error', 'message': f'Error fetching user details: {str(e)}'},
                            status=status.HTTP_400_BAD_REQUEST)

        user_profile_details = {
            'user_name': f"{user_profile.firstname} {user_profile.lastname}",
            'profile_photo': user_profile.profilephoto,
    ...
    def post(self, request):
        ...
        try:
            ...
            with transaction.atomic():
                try:
                    role_mapping = RoleMapping.objects.filter(user=user, role__id=role_type).first()
